...
rates_calculator = coaldecoder.PairCoalescenceRates(ts, sample_sets, time_breaks, sample_set_names=population_names, bootstrap_blocks=None) #windows)
for i in range(2, 11):
    tmp = coaldecoder.PairCoalescenceRates(f"/sietch_colab/natep/trio-coal/sims/osc/oscillating_{i}.ts", sample_sets, time_breaks, sample_set_names=population_names, bootstrap_blocks=windows)
...
_, _, time_grid, exp_rates = rates_and_demography(pairs_only=True)

print(exp_rates.shape, emp_rates.shape)

import matplotlib.pyplot as plt
fig, axs = plt.subplots(exp_rates.shape[0], 1, figsize=(2*4, exp_rates.shape[0]*4))
axs[0].plot(time_grid[:-1], exp_rates[0])
axs[1].plot(time_grid[:-1], exp_rates[1])
axs[2].plot(time_grid[:-1], exp_rates[2])
axs[0].step(time_breaks[:-1], emp_rates[0], color='black', where='post')
axs[1].step(time_breaks[:-1], emp_rates[1], color='black', where='post')
axs[2].step(time_breaks[:-1], emp_rates[2], color='black', where='post')
plt.tight_layout()
plt.savefig("/home/natep/public_html/trio-pres/tmp/true_pair_rates_check.png")
